...
class Environment {
  ...
  dynamic _obtainCurrentEnv(String attr, {bool isDirect = false}) {
    if(isDebug) {
      ...
      defaultCount++;
      if(attr == "isDirectBreak" && defaultCount == 50)
        int x=10;
    }

    if(store.containsKey(attr))
      return this;
    ...
    if(isDirect == false) {
      if(store.containsKey(WTVMConstant.superKeyword)) {
        ...
        else if(superPointer is Environment) {
          superEnv = superValue;
        }
  ...
  dynamic get(String attr, {bool isDirect = false}) {
    ...
    if(attr == null)
      throw "The obtain attribute value is null";
    ...
    if(tempValue is Environment) {
      ...
      if(value == null && isDirect == false) {
        if(curEnv != null && curEnv.store.containsKey(WTVMConstant.thisKeyword)) {
  ...
  void set(String attr, dynamic object, {bool isDirect = false, bool isOverride = true}) {
    ...
    if(attr == null || attr == "")
      debugError("Variable name cannot be null");
